# version 3b matches by means of the means (lines) across the weeks minus Sunday (weekday = 1)
...
#sample usage: week.facet3b(dataset= labor.agg, job.or.all = "SERVER", light.color = "light blue", dark.color = "dark blue", compare.sales = TRUE)
#
...
hour.plot.data <- merge(hour.plot.data, days.df, all.x = TRUE, all.y = FALSE)  
  ...
  if(compare.sales == TRUE){
    ...
    scale.ratio <- (mean(hour.plot.data$mean.i/60, na.rm = TRUE))/(mean(sales.quartiles$mean.i, na.rm = TRUE))
