...
#### version _c creates the ability to filter by 2 or more job ids.  Previously this was limited to 1 or all.  _b attempted this solution but didn't re-aggregate, so quantiles were based on too many rows.

##### Completed new version pulling in the correct quantile files 
##### Also uses new quantile files but need to check what comment changes are needed
...
week.facet <- function(dataset, job.or.all = "all", light.color = "light blue", dark.color = "blue",
  ...
  if(job.or.all != "all"){
    dataset <- filter(dataset, LONGNAME %in% job.or.all)
    dataset <- aggregate(MINUTES ~ Date.Time + Date + weekday + hour, dataset, sum)
    dataset$LONGNAME <- job.or.all[1]
...
hour.plot.data <- merge(hour.plot.data, days.df, all.x = TRUE, all.y = FALSE)  
  ...
  if(compare.sales == TRUE){
    ...
    if(dollars.checks.items == "checks"){
      sales.quartiles <- read.csv("data/quants17checks2018-09-06.csv", stringsAsFactors = FALSE)
    } else if(dollars.checks.items == "dollars"){
      sales.quartiles <- read.csv("data/quants17dollars2018-09-06.csv", stringsAsFactors = FALSE)
    } else if(dollars.checks.items == "items"){
      sales.quartiles <- read.csv("data/quants17items.file2018-09-06.csv", stringsAsFactors = FALSE)
    }
...
##Servers and Head Servers

This first chart shows how many SERVERS and HEAD SERVERS are working on average at each hour of the day. All the charts have some form of a blue "M", although the M isn't as clear with head servers included.  The two spikes in the M represent lunch and dinner.  The first one on the far left is for Mondays (in 2017). It shows that you average 5 servers at dinner.  The light blue shading provides additional information because every day doesn't hit the average exactly.  The light blue shows the range where the number will be 75% of the time.  For chart second from the left representing Tuesday, this means you average 4 servers at dinner, and at least 75% of the time you have either 4 or 5 servers.  It's also fine to just look at the blue Ms and ignore the shaded intervals if you prefer.
...
week.facet(dataset =labor.agg, job.or.all = c("SERVER","HEAD SERVER"), compare.sales = FALSE)
...
This chart overlays dollars per hour on top of the servers per hour chart from above.  Green is sales, throughout this document.  The scale for the dollars is on the right side.  For Monday, both lunch and dinner peak at about $500 per hour on average.  Again, there's a shaded area for the green also, which shows where the sales are 75% of the time

####Observations
- Wednesday and Thursday are staffed with servers almost exactly like Monday (same shaped blue M), but dinner time on those days is much larger.  Would more servers allow for Wednesday and Thursday dinners to be even larger?
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height=4, fig.width=7.7}
setwd("C://Users/The Pritchard family/Documents/R/christophers/")
week.facet(dataset = labor.agg, job.or.all = c("SERVER","HEAD SERVER"), compare.sales = TRUE, dollars.checks.items = "dollars")
